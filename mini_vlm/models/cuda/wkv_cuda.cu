// WKV time-mixing recurrence, fused CUDA kernel.
//
// Ported from BlinkDL/RWKV-LM (https://github.com/BlinkDL/RWKV-LM),
// RWKV-v4/cuda/wkv_cuda.cu, licensed Apache-2.0. Math and kernel structure
// are unchanged from the original; this file only differs in comments.
//
// One thread handles one (batch, channel) pair and loops sequentially over
// T, so it does exactly what mini_vlm.models.rwkv.RWKVTimeMix's Python
// `for t in range(T)` loop does -- just without per-step Python/kernel
// dispatch overhead. See mini_vlm/verify_wkv_cuda.py to check the two
// agree numerically before trusting this on real training.
#include <stdio.h>
#include <assert.h>
